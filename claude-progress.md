...
- 日期：2026-08-24
- 本輪目標：使用者安裝工作階段 036 的 APK 後回報第七個問題——很多明明有使用次數的單品被標示成「尚未使用」，今天才使用過的單品卻標示「7天前使用過」
- 已完成：
  - 排查確認第一個現象的根因：工作階段 033 為了解決 'manual' 標籤語意混用的問題（同時代表『真實日期』跟『沒有日期依據的補插』），選擇整批排除 'manual'，理論上最「乾淨」，但沒有意識到實務衝擊——這個 App 的「手動改使用次數」功能（items-001）從 2026-08-10 就上線，一路到工作階段 033（2026-08-24）才把來源標籤拆成 'count-sync'／'manual-log'，中間整整兩週左右，使用者只要用這個功能追蹤穿搭，背後紀錄全部是 'manual' 標籤；排除它等於把使用者兩週份的真實使用歷史整批清空，範圍遠比預期嚴重
  - `src/services/usageLogService.ts` 的 `getLastUsedDates()` 改回信任 `'manual'`，現在只排除 `'migration'`——`'migration'` 是唯一在設計上從一開始就不可能是真實日期的來源（v3→v4 migration 寫入時本來就是「找不到真正歷史、用購買日期隨便湊」），跟 `'manual'` 至少有機會是真實編輯日期的情況本質不同
  - 這是這整個系列問題裡第一次承認「之前為了理論上的資料純粹性做的排除，實際上造成的傷害比它想解決的問題更大」；記錄教訓：以後遇到「這個來源的日期不是 100% 可靠」的情況，要優先評估「排除它會影響多少既有真實資料」，不能只看「這個來源理論上不夠純粹」就直接排除
  - 更新測試：`src/__tests__/services/usageLogService.test.ts` 的 `getLastUsedDates` 查詢條件斷言更新為包含 `'manual'`
  - 第二個現象（今天用過卻顯示7天前）這輪沒有找到確切根因，只能推測跟工作階段 036 修的「usage_count 與真實 log 筆數對不齊」是同一類問題的另一個變形，但無法在沒有裝置端資料的情況下進一步確認
  - 更新 feature_list.json 的 ranking-001 evidence／notes，記錄這次的排查、修法、以及尚未解決的第二個問題
  - 重新本機建置 Android release APK（sparkwear-v2.0.0-20260824-1903.apk）並上傳 Google Drive
- 執行過的驗證：`pnpm test`（24 suites、372 tests 全過）；`npx tsc --noEmit -p .`（無新增型別錯誤，既有 outfits/form.tsx 錯誤與本次改動無關）；`./gradlew assembleRelease` 建置成功
- 已知風險或未解決問題：本次改動只做過自動化檢查與建置成功，**尚未經過使用者實機驗證這次是否真的讓大量誤判成尚未使用的單品變回正常**；**「今天用過卻顯示7天前」這個問題這輪沒有修好**，需要使用者提供更具體的重現細節（例如是哪件單品、購買日期、今天具體做了什麼操作）才能繼續排查；順帶發現一個尚未處理的潛在問題——`src/services/itemService.ts` 的 `referenceDate`／多處畫面的 `TODAY` 常數都用 `new Date().toISOString().slice(0,10)`，這是 UTC 日期不是本機時區日期，使用者在台灣（UTC+8）如果在當地時間凌晨 0:00～7:59 之間操作，記錄到的日期會是前一天，屬於已知但尚未修的邊界案例，不足以解釋 7 天的落差、但值得之後留意
- 下一步最佳動作：等使用者用新 APK（sparkwear-v2.0.0-20260824-1903.apk）實機確認大量單品「尚未使用」誤判是否消失；如果「今天用過卻顯示7天前」的問題仍然存在，需要請使用者針對那件特定單品提供更多細節才能繼續排查，不要再憑空猜測；如果這輪之後主要問題都解決，這已經是第七輪修復，之後如果還有新一波「未使用天數」相關回報，要認真考慮是否該用更根本的方式重新設計這個指標（例如乾脆不做「未使用天數」的日期猜測，只呈現「有無使用紀錄」與「使用次數」，避免持續在猜測日期準確性這件事上打轉）

### 工作階段 036

- 日期：2026-08-24
- 本輪目標：使用者安裝工作階段 035 的 APK 後回報第六個問題——今天明明在「手動增加使用次數」那邊增加過，卻還是顯示「尚未使用」
- 已完成：
  - 排查找到根因：工作階段 034 新增的 `reseedMissingOutfitLogs`（v6→v7 migration）只補 `item_usage_logs` 表、沒有同步調高對應的 `items.usage_count`，導致部分單品的「真實 log 筆數」已經超過 `usage_count` 欄位；`reconcileUsageLogs()` 是靠比較「usage_count 目標值」跟「item_usage_logs 目前筆數」的差來決定要不要新增紀錄，一旦真實筆數已經比 usage_count 高，使用者手動把 usage_count 往上調（例如 +1）算出來的差可能還是負的或零，完全不會新增今天的 count-sync 紀錄，看起來就像「明明加了次數卻還是顯示尚未使用」
  - `src/services/usageLogService.ts` 新增 `syncUsageCountToLogCount(db)`：找出 `item_usage_logs` 真實筆數超過 `items.usage_count` 的單品，把 `usage_count` 補回去對齊真實筆數；只處理「真實筆數較高」這個方向，不會把使用者刻意調低的 usage_count 往下拉
  - `src/db/index.ts` 的 `runMigrations()` 新增 `current < 8` 區塊，接到既有 migration 機制當 v7→v8
  - 新增測試：`src/__tests__/services/usageLogService.test.ts` 的 `syncUsageCountToLogCount`（3 項：正確調高並回傳受影響筆數、usage_count 已經對齊或更高時不做任何事、SQL 條件正確鎖定「真實筆數 > usage_count」這個方向）
  - 更新 feature_list.json 的 ranking-001 evidence／notes，記錄這次的排查、修法，以及一個使用者需要知道的可見副作用
  - 重新本機建置 Android release APK（sparkwear-v2.0.0-20260824-1853.apk）並上傳 Google Drive
- 執行過的驗證：`pnpm test`（24 suites、372 tests 全過，含新增 3 項）；`npx tsc --noEmit -p .`（無新增型別錯誤，既有 outfits/form.tsx 錯誤與本次改動無關）；`./gradlew assembleRelease` 建置成功
- 已知風險或未解決問題：本次改動只做過自動化檢查與建置成功，**尚未經過使用者實機驗證這次的修復是否真的讓手動增加使用次數當下能正確反映為「今天使用」**；**這次修復有一個使用者需要事先知道的可見副作用**：部分單品的「使用次數」欄位安裝新版後可能會比之前看到的數字高——這是預期內的修正（代表這件單品原本就有真實的「新增穿搭」紀錄，之前因為工作階段 026 那個舊 bug 在某次手動調整使用次數時被連帶低估了，現在補回真實數字），不是新的異常，需要在回報時先跟使用者說明，避免使用者看到數字變動又誤以為是新 bug
- 下一步最佳動作：等使用者用新 APK（sparkwear-v2.0.0-20260824-1853.apk）實機確認：(1) 今天手動增加使用次數後，未使用天數是否正確變成 0（不再顯示「尚未使用」）(2) 部分單品「使用次數」數字變高是否可以理解為資料修正（已提前說明）(3) 連同前幾輪的所有修復是否都正常；如果這輪確認通過，這已經是「未使用天數」資料層修復的第六輪，之後如果還有問題，要認真考慮是否已經把這個指標的資料依賴關係修到位、或是否需要更根本地重新設計而不是繼續打補丁
